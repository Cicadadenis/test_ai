...
DEFAULT_CONFIG = {
    ...
    "tts_lang": "ru",
    "tts_max_chars": "500",
    # Цены Premium (1/3/5/7/15/30 дней)
    "price_1day": "1.0",
    "price_3day": "3.0",
    "price_5day": "5.0",
    "price_7day": "7.0",
    ...
    "groq_alert_sent": "0",
    "tavily_alert_sent": "0",
    # Напоминания о замене токенов (через сколько дней)
    "reminder_groq_token": "30",
    "reminder_groq_token_2": "30",
    "reminder_groq_token_3": "30",
    "reminder_tavily_token": "30",
    "reminder_last_check": "",
}
...
AVAILABLE_MODELS = [
    ...
    "deepseek-r1-distill-llama-70b",      # reasoning, сложные задачи
    "qwen-qwq-32b",                       # reasoning, математика и код
    "gemma2-9b-it",                       # быстрая и лёгкая
]

MODEL_INFO = {
    "llama-3.1-8b-instant":                     "⚡ Быстрый · 6K TPM · 14.4K req/day",
    "llama-3.3-70b-versatile":                   "🧠 Умный · 12K TPM · 1K req/day",
    "qwen/qwen3-32b":                            "💻 Код · 60 req/min",
    "meta-llama/llama-4-scout-17b-16e-instruct": "🔭 Свежий · 1K req/day",
    "deepseek-r1-distill-llama-70b":             "🔬 Reasoning · сложные задачи",
    "qwen-qwq-32b":                              "🔢 Reasoning · математика и код",
...
def _get_groq_client() -> Groq:
    """Возвращает Groq клиент со следующим ключом по кругу (thread-safe через глобальный счётчик)."""
    global _groq_key_index
    # Атомарное чтение+инкремент — в CPython int += 1 атомарен, asyncio однопоточен
    idx = _groq_key_index
    _groq_key_index = (idx + 1) % len(_GROQ_KEYS)
    return Groq(api_key=_GROQ_KEYS[idx])
...
async def call_ollama(api_messages: list) -> str | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=120) as http:
            ...
            data = resp.json()
            return re.sub(r'<think>.*?</think>', '', data["message"]["content"], flags=re.DOTALL).strip()
    except Exception as e:
...
async def compress_history(user_id: int):
    ...
    try:
        ...
        summary = re.sub(r'<think>.*?</think>', '', chat.choices[0].message.content, flags=re.DOTALL).strip()
    except Exception as e:
...
def trim_messages_to_token_limit(messages: list, max_chars: int = 4000) -> list:
    ...
    mid_msgs = other_msgs[:-1]
    while mid_msgs:
        total = sum(len(m.get("content", "")) for m in system_msgs + mid_msgs + [last_msg])
        if total <= max_chars:
            break
        mid_msgs.pop(0)
    return system_msgs + mid_msgs + [last_msg]
...
def md_to_html(text):
    ...
    # Убираем <think>...</think> до экранирования (иначе escape_html ломает regex)
...
def sanitize_html(text: str) -> str:
    ...
    # Первый проход: убираем несовпадающие closing-теги
    def _strip_bad_closing(m):
        ...
        if closing:
            if stack and stack[-1] == tag:
                ...
                return ""  # убираем несовпадающий closing-тег
        else:
            stack.append(tag)
            return m.group(0)
    text = tag_pattern.sub(_strip_bad_closing, text)
...
# ════════════════════════════════════════════════════
PLAN_LABELS = {
    "day": "1 день", "week": "1 неделя", "month": "1 месяц",
    "1day": "1 день", "3day": "3 дня", "5day": "5 дней",
    "7day": "7 дней", "15day": "15 дней", "30day": "30 дней"
}
...
async def create_invoice(user_id: int, plan: str, currency: str) -> dict | None:
    ...
    price_map = {
        ...
        "1day":  cfg_float("price_1day"),
        "3day":  cfg_float("price_3day"),
        "5day":  cfg_float("price_5day"),
        "7day":  cfg_float("price_7day"),
        "15day": cfg_float("price_15day"),
        "30day": cfg_float("price_30day"),
    }
    ...
    payload = {
        ...
        "asset":          currency,
        "amount":         str(amount),
        "description":    f"Cicada-AI Premium — {PLAN_LABELS.get(plan, plan)}",
...
def add_balance_keyboard():
    """Клавиатура покупки Premium — 1/3/5/7/15/30 дней."""
    p1 = cfg_float("price_1day")
    p3 = cfg_float("price_3day")
    p5 = cfg_float("price_5day")
    p7 = cfg_float("price_7day")
    p15 = cfg_float("price_15day")
    p30 = cfg_float("price_30day")
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(f"1️⃣ День — ${p1}",  callback_data="pay_days_1"),
            InlineKeyboardButton(f"3️⃣ Дня — ${p3}",  callback_data="pay_days_3"),
        ],
        [
            InlineKeyboardButton(f"5️⃣ Дней — ${p5}",  callback_data="pay_days_5"),
            InlineKeyboardButton(f"7️⃣ Дней — ${p7}",  callback_data="pay_days_7"),
        ],
        [
            InlineKeyboardButton(f"1️⃣5️⃣ Дней — ${p15}", callback_data="pay_days_15"),
            InlineKeyboardButton(f"3️⃣0️⃣ Дней — ${p30}", callback_data="pay_days_30"),
        ],
        [InlineKeyboardButton("◀️ Назад", callback_data="subscription_menu")],
    ])
...
def admin_reminders_keyboard():
    """Настройки напоминаний о замене токенов."""
    g1 = CFG.get("reminder_groq_token", "30")
    g2 = CFG.get("reminder_groq_token_2", "30")
    g3 = CFG.get("reminder_groq_token_3", "30")
    tv = CFG.get("reminder_tavily_token", "30")
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🔑 GROQ_TOKEN: {g1} дн",     callback_data="admin_reminder_groq_token")],
        [InlineKeyboardButton(f"🔑 GROQ_TOKEN_2: {g2} дн",   callback_data="admin_reminder_groq_token_2")],
        [InlineKeyboardButton(f"🔑 GROQ_TOKEN_3: {g3} дн",   callback_data="admin_reminder_groq_token_3")],
        [InlineKeyboardButton(f"🔑 TAVILY_TOKEN: {tv} дн",   callback_data="admin_reminder_tavily_token")],
...
def admin_tokens_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔑 Сменить ADMIN_ID",      callback_data="admin_set_admin_id")],
        [InlineKeyboardButton("🔑 Сменить GROQ_TOKEN",     callback_data="admin_set_groq_token")],
        [InlineKeyboardButton("🔑 Сменить GROQ_TOKEN_2",   callback_data="admin_set_groq_token_2")],
        [InlineKeyboardButton("🔑 Сменить GROQ_TOKEN_3",   callback_data="admin_set_groq_token_3")],
        [InlineKeyboardButton("🔑 Сменить CRYPTOBOT_TOKEN", callback_data="admin_set_cryptobot_token")],
        [InlineKeyboardButton("🔑 Сменить TAVILY_TOKEN",   callback_data="admin_set_tavily_token")],
        [InlineKeyboardButton("🔑 Сменить HORDE_TOKEN",    callback_data="admin_set_horde_token")],
...
def admin_prices_keyboard():
    """Клавиатура для изменения цен Premium — 1/3/5/7/15/30 дней."""
    p1 = cfg_float("price_1day")
    p3 = cfg_float("price_3day")
    p5 = cfg_float("price_5day")
    p7 = cfg_float("price_7day")
    ...
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(f"1️⃣ День: ${p1}", callback_data="admin_setprice_1day"),
            InlineKeyboardButton(f"3️⃣ Дня: ${p3}", callback_data="admin_setprice_3day"),
        ],
        [
            InlineKeyboardButton(f"5️⃣ Дней: ${p5}", callback_data="admin_setprice_5day"),
            InlineKeyboardButton(f"7️⃣ Дней: ${p7}", callback_data="admin_setprice_7day"),
        ],
        [
            InlineKeyboardButton(f"1️⃣5️⃣ Дней: ${p15}", callback_data="admin_setprice_15day"),
            InlineKeyboardButton(f"3️⃣0️⃣ Дней: ${p30}", callback_data="admin_setprice_30day"),
        ],
        [InlineKeyboardButton("◀️ Назад", callback_data="admin_bot_settings")],
    ])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_premium(user_id) and user_id != ADMIN_ID:
        ...
        try:
            ...
            exp = until
        status_block = (
            f"💎 <b>Premium</b> до {exp}\n"
            f"📊 Лимит: <b>{left:,} / {lim:,}</b>\n"
            f"🟢 Статус: <b>Готов к работе</b>"
        )
    else:
        status_block = (
    ...
    name = escape_html(user.first_name or "разработчик")
    text = (
        f"👋 Добро пожаловать, {name}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🚀 <b>Cicada AI v4.5</b> — умный помощник для задач и разработки\n"
        f"⚡ Быстро • Локально • Безопасно\n\n"
        f"{status_block}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🧠 <b>Возможности</b>\n\n"
        f"💻 <b>Кодинг</b> — генерация, исправление, API и боты\n"
        f"🔧 <b>DevOps</b> — Docker, Linux, автоматизация\n"
        f"📎 <b>Файлы</b> — анализ логов и документов\n"
        f"🔍 <b>Интернет</b> — поиск и проверка данных\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⚡ <b>Быстрый старт</b>\n\n"
        f"Напиши задачу\n"
        f"или отправь файл — начнём работу.\n"
        f"━━━━━━━━━━━━━━━━━━━━━━"
    )
    await update.message.reply_text(
        text,
...
async def process_file_in_chunks(message, context, user_id: int, user_input: str, filename: str, content: str):
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        system_msg = (
            f"Ты — Cicada AI, AI-ассистент для разработчиков, созданный компанией Cicada3301 на технологии Cicada AI. "
        ...
        try:
            ...
            for _attempt in range(len(_GROQ_KEYS) or 1):
                ...
                try:
                    ...
                    result = re.sub(r'<think>.*?</think>', '', chat.choices[0].message.content, flags=re.DOTALL).strip()
                    last_exc = None
                    break
    ...
    final_system = (
        f"Ты — Cicada AI, созданный компанией Cicada3301 на технологии Cicada AI. Собери итоговый ответ на запрос пользователя "
        f"по файлу «{filename}» на основе анализа всех частей. "
        f"Запрос: {user_input}"
    )

    # Финальная сборка всегда через summary_model (высокий TPM лимит)
    # qwen3-32b и другие модели с малым TPM не подходят для большого контекста
    _final_model = CFG.get("summary_model", "llama-3.1-8b-instant")

    # Оцениваем размер: ~4 символа = 1 токен, резервируем 1500 на system + ответ
    # При лимите 6000 TPM оставляем не более 3500 токенов на контент ≈ 14000 символов
    _MAX_CONTENT_CHARS = 14000
    joined = "\n\n".join(partial_results)
    if len(joined) > _MAX_CONTENT_CHARS:
        # Обрезаем равномерно: берём начало и конец, пропускаем середину
        half = _MAX_CONTENT_CHARS // 2
        joined = joined[:half] + "\n\n...[середина пропущена из-за ограничений токенов]...\n\n" + joined[-half:]
    ...
    try:
        last_exc = None
        for _attempt in range(len(_GROQ_KEYS) or 1):
            _client_final = _get_groq_client()
            def _call_final(_c=_client_final, _msgs=final_messages, _mdl=_final_model):
                return _c.chat.completions.create(
                    model=_mdl,
                    messages=_msgs,
                    max_tokens=2048,
            ...
            try:
                ...
                final_reply = re.sub(r'<think>.*?</think>', '', chat.choices[0].message.content, flags=re.DOTALL).strip()
                last_exc = None
                break
            except Exception as _e:
                last_exc = _e
                err_str = str(_e)
                is_413 = "413" in err_str or ("tokens" in err_str and "limit" in err_str.lower())
                if is_413:
                    # Ещё раз обрезаем контент вдвое и повторяем
                    _MAX_CONTENT_CHARS //= 2
                    half = _MAX_CONTENT_CHARS // 2
                    cut = joined[:half] + "\n\n...[обрезано]...\n\n" + joined[-half:]
                    final_messages[1]["content"] = cut
                    wait = 0
                else:
                    wait = 15 if "429" in err_str or "rate_limit" in err_str.lower() else 3
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ════════════════════════════════════════════════════
    #  TRIAL VERSION — File reading is Premium only
    # ════════════════════════════════════════════════════
    premium_message = """⭐️ <b>ЧТЕНИЕ ФАЙЛОВ — PREMIUM ФУНКЦИЯ</b>

🔒 <b>Текущий статус:</b> Trial версия

Функция чтения и интеллектуального анализа файлов
доступна только пользователям с активной <b>Premium подпиской</b>.

─────────────

💎 <b>Что открывает Premium:</b>

📁 Загрузка файлов до <b>500 КБ</b>
📄 Глубокий анализ кода и документов
🔍 Поиск ошибок и автоматическое исправление
💾 Сохранение файлов и истории работы
⚡ Приоритетная скорость обработки
🧠 Расширенные возможности AI

─────────────

🚀 <b>Обновитесь до Premium, чтобы разблокировать доступ</b>

📩 Подключение подписки:
👤 Администратор: <b>@satanasat</b>"""
    await update.message.reply_text(
        premium_message,
        parse_mode=ParseMode.HTML,
        reply_markup=main_keyboard(user_id),
...
# ════════════════════════════════════════════════════
#  🖼 ГЕНЕРАЦИЯ ИЗОБРАЖЕНИЙ — Stable Horde
# ════════════════════════════════════════════════════
HORDE_TOKEN = os.getenv("HORDE_TOKEN", "0000000000")  # анонимный если не задан
...
async def generate_image(prompt: str) -> tuple[bytes | None, str]:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            # 1. Отправляем задачу
            resp = await client.post(f"{HORDE_API}/generate/async", headers=headers, json=payload)
            if resp.status_code != 202:
                return None, f"Ошибка отправки задачи {resp.status_code}: {resp.text[:200]}"
            job_id = resp.json().get("id")
            if not job_id:
                return None, "Не получен ID задачи"

            # 2. Ждём результат (до 3 минут)
            for _ in range(36):
                await asyncio.sleep(5)
                check = await client.get(f"{HORDE_API}/generate/check/{job_id}", headers=headers)
                data = check.json()
                if data.get("done"):
                    break
                if data.get("faulted"):
                    return None, "Задача завершилась с ошибкой на сервере Horde"
            else:
                return None, "Превышено время ожидания (3 мин)"

            # 3. Получаем результат
            status = await client.get(f"{HORDE_API}/generate/status/{job_id}", headers=headers)
            generations = status.json().get("generations", [])
            if not generations:
                return None, "Нет результатов генерации"

            img_url = generations[0].get("img")
            if not img_url:
                return None, "Нет URL изображения в ответе"

            # 4. Скачиваем картинку
            img_resp = await client.get(img_url)
            if img_resp.status_code == 200:
                return img_resp.content, ""
            return None, f"Ошибка загрузки картинки: {img_resp.status_code}"

    except httpx.TimeoutException:
        return None, "Таймаут соединения. Попробуй ещё раз"
    except Exception as e:
        return None, str(e)


async def handle_imagegen(message, prompt: str):
    """Обрабатывает запрос на генерацию изображения с прогресс-баром."""
    ...
    async def _animate():
        frames = [
            ("🖼 <b>Генерирую изображение...</b>", 10),
            ("🖼 <b>Генерирую изображение...</b>", 25),
            ("🖼 <b>Генерирую изображение...</b>", 42),
            ("🖼 <b>Генерирую изображение...</b>", 58),
            ("🖼 <b>Генерирую изображение...</b>", 73),
            ("🖼 <b>Генерирую изображение...</b>", 88),
        ]
        i = 0
    ...
    try:
        img_bytes, error = await generate_image(prompt)
        stop_anim.set()
        anim_task.cancel()
        try:
            await wait_msg.edit_text(
        ...
        try:
            ...
            pass
        await message.reply_text(f"❌ Ошибка: {escape_html(str(e))}", parse_mode=ParseMode.HTML)
...
def extract_files_from_reply(reply: str) -> dict:
    ...
    Ищет имя файла:
    1. В строках перед блоком кода (до 5 строк)
    2. В первых 3 строках внутри блока (комментарии: # file.py, // file.js и т.д.)
    3. Fallback: генерирует имя по языку
    """
    files = {}
    lines = reply.split('\n')
    i = 0
    ...
    # Паттерн метки: "💻 Код [filename.ext]" или любая строка с [filename.ext]
    _label_re = re.compile(r'\[([\w\-/]+\.\w{1,6})\]')

    while i < len(lines):
        # Обычный блок с тройными бэктиками
        if lines[i].startswith('```'):
            ...
            code_blocks.append({'lang': lang, 'code': code, 'prev_lines': prev_lines, 'label_line': ''})

        # Голый блок кода после метки [filename.ext] без бэктиков:
        # Следующая строка после метки — не ```, значит код идёт напрямую
        elif _label_re.search(lines[i].strip()):
            label_line = lines[i].strip()
            prev_lines = [lines[j].strip() for j in range(max(0, i - 5), i + 1)]
            # Смотрим: следующая непустая строка — ``` или нет?
            j = i + 1
            while j < len(lines) and lines[j].strip() == '':
                j += 1
            if j < len(lines) and lines[j].startswith('```'):
                # Бэктик-блок — обработается на следующей итерации
                i += 1
                continue
            # Голый код: собираем до следующей метки [*.ext], конца или обычного текста
            _code_prefixes = ('pip', 'python', 'node', 'npm', 'npx', 'apt', 'cd ', 'export',
                              'curl', 'wget', 'mkdir', 'cp ', 'mv ', 'rm ', 'echo', 'cat ',
                              '#', '//', 'import', 'from ', 'def ', 'class ', 'const ', 'let ',
                              'var ', 'function', 'if ', 'for ', 'while ', 'return', 'print')
            code_lines = []
            k = i + 1
            while k < len(lines):
                if _label_re.search(lines[k].strip()) and lines[k].strip() != label_line:
                    break
                if lines[k].startswith('```'):
                    break
                cur = lines[k].strip()
                # Непустая строка с кириллицей и без признаков кода — обычный текст, стоп
                if cur and code_lines:
                    has_cyrillic = bool(re.search(r'[а-яёА-ЯЁ]', cur))
                    has_code_chars = bool(re.search(r'[=\(\){}\[\]<>\|\\]', cur))
                    starts_like_code = cur.startswith(_code_prefixes)
                    if has_cyrillic and not has_code_chars and not starts_like_code:
                        break
                # Пустая строка — смотрим что дальше
                if not cur:
                    nxt = k + 1
                    ...
                    if nxt < len(lines):
                        ...
                        if not _label_re.search(nxt_line) and not nxt_line.startswith('```'):
                            is_plain = bool(re.search(r'[а-яёА-ЯЁ]', nxt_line)) or \
                                       (len(nxt_line) > 20 and not re.search(r'[=\(\){}\[\]<>]', nxt_line))
                            if is_plain:
                                break
            ...
            code = '\n'.join(code_lines).strip()
            if code:
                # Определяем lang по расширению из метки
                m = _label_re.search(label_line)
                ext_from_label = m.group(1).rsplit('.', 1)[-1].lower() if m else ''
                lang = ext_from_label
                code_blocks.append({'lang': lang, 'code': code, 'prev_lines': prev_lines, 'label_line': label_line})
            i = k
            continue
    ...
    valid_exts = {
        'py', 'js', 'ts', 'html', 'htm', 'css', 'php', 'sh', 'bash',
        'json', 'xml', 'yaml', 'yml', 'sql', 'cpp', 'c', 'h', 'java',
        'go', 'rs', 'rb', 'md', 'txt', 'env', 'toml', 'ini', 'cfg',
    }

    def is_valid_filename(name: str) -> bool:
        """Проверяет что строка — настоящее имя файла, а не CSS-значение типа 0.5rem."""
        if not name or '.' not in name:
            return False
        ext = name.rsplit('.', 1)[-1].lower()
        if ext not in valid_exts:
            return False
        # Имя не должно начинаться с цифры (0.5rem, 2px и т.п.)
        if name[0].isdigit():
            return False
        return True

    # Гибкий regex: ловит имена с точками в середине (models.admin.py), подчёркиваниями
    _fname_re_bracket = re.compile(r'\[([A-Za-z0-9_\-\.]+?\.\w{1,6})\]')
    _fname_re_plain   = re.compile(r'([A-Za-z0-9_\-\.]+?\.\w{1,6})')

    for idx, block in enumerate(code_blocks):
        lang     = block['lang'].lower()
        code     = block['code']
        prev_lines = block['prev_lines']
        label_line = block.get('label_line', '')
        fname    = None

        # 1. Приоритет — комментарии ВНУТРИ блока (# filename: ..., // file.js, <!-- x.html -->)
        for line in code.split('\n')[:5]:
            stripped = line.strip()
            # # filename: shop.py  /  // filename: shop.js
            m = re.search(r'(?:file(?:name)?|name)\s*[:=]\s*([A-Za-z0-9_\-\.]+?\.\w{1,6})', stripped, re.IGNORECASE)
            if m:
                candidate = m.group(1).split('/')[-1]
                if is_valid_filename(candidate): fname = candidate; break
            # HTML: <!-- index.html -->
            m = re.match(r'^<!--\s*([A-Za-z0-9_\-\./]+?\.\w{1,6})', stripped)
            if m:
                candidate = m.group(1).split('/')[-1]
                if is_valid_filename(candidate): fname = candidate; break
            # # shop.py  //  scripts/shop.js  /* style.css */
            m = re.match(r'^(?:#|//|/\*|--)\s*([A-Za-z0-9_\-\./]+?\.\w{1,6})', stripped)
            if m:
                candidate = m.group(1).split('/')[-1]
                if is_valid_filename(candidate): fname = candidate; break

        # 2. label_line — строка метки вида "💻 Код [models.admin.py]"
        if not fname and label_line:
            m = _fname_re_bracket.search(label_line)
            if m:
                candidate = m.group(1).split('/')[-1]
                if is_valid_filename(candidate): fname = candidate

        # 3. prev_lines — строки перед блоком
        if not fname:
            for prev in reversed(prev_lines):
                clean = re.sub(r'[*#`]', '', prev).strip()
                # Сначала в квадратных скобках
                m = _fname_re_bracket.search(clean)
                if m:
                    candidate = m.group(1).split('/')[-1]
                    if is_valid_filename(candidate): fname = candidate; break
                # Потом просто имя файла
                m = _fname_re_plain.search(clean)
                if m:
                    candidate = m.group(1).split('/')[-1]
                    if is_valid_filename(candidate): fname = candidate; break

        # 4. Fallback — генерируем имя
        if not fname:
            ext = ext_map.get(lang, '.txt')
            fname = f'code{idx+1}{ext}'
...
# ════════════════════════════════════════════════════
def format_ai_response(text: str, style: str = "premium") -> str:
    """
    Форматирует ответ AI с красивым оформлением.
    
    Варианты стилей:
    - premium: Премиальный SaaS стиль
    - minimal: Минималистичный
    - detailed: Подробный с разделением
    """
    text = text.strip()
    
    if style == "premium":
        return f"""🧠 *Cicada AI ответил:*
...
• ⚡ Ответ сгенерирован AI"""
    
    elif style == "detailed":
        # Добавляем смайлы к пунктам если их нет
        lines = text.split('\n')
        formatted_lines = []
        for line in lines:
            stripped = line.strip()
            if stripped.startswith(('•', '-', '*', '1.', '2.', '3.', '4.', '5.', '6.', '7.', '8.', '9.')):
                if not stripped.startswith('•'):
                    line = '• ' + stripped.lstrip('- *123456789.')
...
def highlight_definition(text: str) -> str:
    """
    Автоматически выделяет определения в тексте.
    Если первая строка содержит '— это' — добавляет эмодзи определения.
    """
    lines = text.split("\n")
    
    if lines and "— это" in lines[0]:
        lines[0] = f"📘 *Определение*\n\n{lines[0]}"
    
    # Также ищем другие паттерны определений
    for i, line in enumerate(lines[:5]):  # Только в начале текста
        stripped = line.strip().lower()
        if any(word in stripped for word in ['это когда', 'это процесс', 'это система', 'это метод']):
...
def add_status_bar(text: str, time_sec: float, from_cache: bool = False, search_used: bool = False) -> str:
    """
    Добавляет красивый статус-бар в конец ответа.
    
    Показывает:
    - Время генерации
    - Было ли из кэша
    - Использовался ли поиск
    """
    cache_emoji = "⚡ *cached*" if from_cache else "🤖 AI"
    search_emoji = " 🔍 *web*" if search_used else ""
    
    return f"""
{text}

━━━━━━━━━━━━━━
⏱ {time_sec:.2f} сек · {cache_emoji}{search_emoji}"""


def format_code_response(code: str, filename: str = None, lang: str = "") -> str:
    """
    Форматирует код с красивым заголовком.
    """
    if filename:
        header = f"💻 *Код* `{filename}`"
    elif lang:
        header = f"💻 *Код* `{lang}`"
    else:
        header = "💻 *Код:*"
...
async def send_ai_response(message, context, user_id, user_input, bypass_cache=False):
    ...
    if needs_web_search(user_input) and not bypass_cache:
        if not has_search_limit(user_id):
            ...
            return
        search_tag = " 🔍"
        wait_msg = await message.reply_text("💬 Cicada AI......")
    ...
    system_msg = (
        "Ты — Cicada AI, AI-ассистент для разработчиков, созданный компанией Cicada3301. "
        "Ты работаешь на технологии Cicada AI. "
        "ВАЖНО: если тебя спрашивают кто тебя создал, кто ты, или на какой технологии работаешь — "
        "всегда отвечай, что ты создан компанией Cicada3301 на основе технологии Cicada AI. "
        "Никогда не упоминай Microsoft, Azure Cognitive Services, Language Generation AI или других компаний как своих создателей. "
        ...
        "ВАЖНО: перед каждым блоком кода ОБЯЗАТЕЛЬНО напиши имя файла в виде строки, например: `shop.py`, `index.html`, `styles.css`. "
    ...
    logger.info(f"[DEBUG] api_messages размер ДО trim: {total_chars} символов, сообщений: {len(api_messages)}")
    api_messages = trim_messages_to_token_limit(api_messages)
    total_chars2 = sum(len(m.get("content","")) for m in api_messages)
    logger.info(f"[DEBUG] api_messages размер ПОСЛЕ trim: {total_chars2} символов, сообщений: {len(api_messages)}")
            ...
            if reply is None:
                ...
                for _attempt in range(len(_GROQ_KEYS) or 1):
                    ...
                    try:
                        ...
                        reply = re.sub(r'<think>.*?</think>', '', chat.choices[0].message.content, flags=re.DOTALL).strip()
                        last_exc = None
                        break
                    except Exception as _e:
                        last_exc = _e
    ...
    # 🔄 Заменяем упоминания Alibaba Cloud на Cicada3301
    reply = re.sub(r'Alibaba Cloud', 'Cicada3301', reply, flags=re.IGNORECASE)

    elapsed = round(time.time() - start_t, 1)
    # Обрезаем длинные ответы AI перед сохранением в историю (чтобы не раздувать контекст)
    MAX_REPLY_IN_HISTORY = 3000
    reply_for_history = reply[:MAX_REPLY_IN_HISTORY] + "\n[...обрезано для контекста]" if len(reply) > MAX_REPLY_IN_HISTORY else reply
    history.append({"role": "user",      "content": user_input})
    ...
    for i, part in enumerate(parts):
        ...
        if part["type"] == "text":
            ...
            # Применяем форматирование только к первому текстовому блоку
            if i == 0:
                # Выбираем стиль в зависимости от типа ответа
                style = "detailed"
                if len(content) <= 280:
                    style = "minimal"
                elif any(kw in content.lower() for kw in ['потому что', 'это значит', 'объяснение', 'суть в том']):
                    style = "premium"
                
                # Применяем форматирование
                content = format_ai_response(content, style=style)
                content = highlight_definition(content)
            
            # Добавляем статус-бар к последнему блоку
            if is_last:
                content = add_status_bar(content, elapsed, from_cache, bool(search_tag))
            ...
            if code_block_num < len(parsed_fnames):
                ...
                # Пробуем извлечь имя из первой строки кода (комментарий)
                _inline_fname = None
                _valid_exts = {'py','js','ts','jsx','tsx','html','htm','css','php','sh','json','xml','yaml','yml','sql','cpp','c','h','java','go','rs','rb','md','txt','toml','ini','cfg'}
                for _line in code.split('\n')[:3]:
                    _s = _line.strip()
                    _m = re.match(r'^(?:<!--\s*|//\s*|#\s*|/\*\s*)[\w\-/]*?([\w\-]+\.\w{1,6})', _s)
                    if _m:
                        _cand = _m.group(1)
                        if not _cand[0].isdigit() and _cand.rsplit('.',1)[-1].lower() in _valid_exts:
                            _inline_fname = _cand
                            break
                if _inline_fname:
                    header = f"💻 <b>Код</b> <code>[{_inline_fname}]</code>"
            ...
            for k, chunk in enumerate(chunks):
                ...
                if is_last_chunk:
                    ...
                    # Используем новый статус-бар для последнего блока
                    timing = f"\n\n━━━━━━━━━━━━━━\n⏱ {elapsed:.2f} сек · {'⚡ cached' if from_cache else '🤖 AI'}{search_tag}"
                await message.reply_text(
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⏰ Настройка напоминаний о замене токенов
    if context.user_data.get("awaiting_reminder"):
        token_name = context.user_data.pop("awaiting_reminder")
        text = update.message.text.strip()
        try:
            val = int(text)
            if val <= 0:
                raise ValueError
            CFG[f"reminder_{token_name}"] = str(val)
            _config_set(f"reminder_{token_name}", str(val))
            await update.message.reply_text(
                f"✅ Напоминание для <code>{token_name.upper()}</code> установлено: <b>{val} дней</b>",
                parse_mode=ParseMode.HTML,
                reply_markup=admin_reminders_keyboard(),
            )
        except ValueError:
            await update.message.reply_text(
                "❌ Введи целое положительное число, например: <code>30</code>",
                parse_mode=ParseMode.HTML,
            )
        return
    ...
    if awaiting and user_id == ADMIN_ID:
        global GROQ_TOKEN, GROQ_TOKEN_2, GROQ_TOKEN_3, TAVILY_TOKEN, BOT_TOKEN, client, _GROQ_KEYS, _groq_key_index
        ...
        elif awaiting == "groq_token_2_input":
            GROQ_TOKEN_2 = text
            _update_env("GROQ_TOKEN_2", text)
            _GROQ_KEYS[:] = [k for k in [GROQ_TOKEN, GROQ_TOKEN_2, GROQ_TOKEN_3] if k]
            await update.message.reply_text(
                f"✅ <b>Groq токен 2 обновлён!</b>\n\nТокен: <code>...{text[-6:]}</code>",
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("◀️ Назад к токенам", callback_data="admin_tokens")],
                ]),
        ...
        elif awaiting == "groq_token_3_input":
            GROQ_TOKEN_3 = text
            _update_env("GROQ_TOKEN_3", text)
            _GROQ_KEYS[:] = [k for k in [GROQ_TOKEN, GROQ_TOKEN_2, GROQ_TOKEN_3] if k]
            await update.message.reply_text(
                f"✅ <b>Groq токен 3 обновлён!</b>\n\nТокен: <code>...{text[-6:]}</code>",
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("◀️ Назад к токенам", callback_data="admin_tokens")],
                ]),
    ...
    if awaiting_tool:
        ...
        if awaiting_tool.startswith("translate_"):
            ...
            return
        elif awaiting_tool == "imagegen":
            await handle_imagegen(update.message, text)
            return
        else:
    ...
    if low_limit is not None:
        ...
        await update.message.reply_text(
            ...
            f"💎 <b>Premium:</b>\n"
            f"1️⃣ День — ${cfg_float('price_1day')}\n"
            f"3️⃣ Дня — ${cfg_float('price_3day')}\n"
            f"5️⃣ Дней — ${cfg_float('price_5day')}\n"
            f"7️⃣ Дней — ${cfg_float('price_7day')}\n"
            f"1️⃣5️⃣ Дней — ${cfg_float('price_15day')}\n"
            f"3️⃣0️⃣ Дней — ${cfg_float('price_30day')}",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ════════════════════════════════════════════════════
        ...
        context.user_data["awaiting_tool"] = "imagegen"
        await query.edit_message_text(
            "🖼 <b>Генерация изображений</b>\n\n"
            "✏️ Опиши что хочешь получить — на русском или английском:\n\n"
            "<i>Например: красивый закат над горами, реалистично</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="tools_menu")]]),
        )
    ...
    # ════════════════════════════════════════════════════
        ...
        # Получаем цену из конфига по количеству дней
        price_key = f"price_{days}day"
        amount = cfg_float(price_key) if price_key in CFG else float(days)
        day_labels = {
            "1": "1 день", "3": "3 дня", "5": "5 дней", "7": "7 дней",
    ...
    # ════════════════════════════════════════════════════
        ...
        """Напоминания о замене токенов."""
        if user_id != ADMIN_ID:
            await answer("⛔ Нет доступа", show_alert=True); return
        await query.edit_message_text(
            f"⏰ <b>Напоминания о замене токенов</b>\n\n"
            f"Настрой через сколько дней напоминать о замене токена.\n\n"
            f"🔑 GROQ_TOKEN: <b>{CFG.get('reminder_groq_token', '30')} дн</b>\n"
            f"🔑 GROQ_TOKEN_2: <b>{CFG.get('reminder_groq_token_2', '30')} дн</b>\n"
            f"🔑 GROQ_TOKEN_3: <b>{CFG.get('reminder_groq_token_3', '30')} дн</b>\n"
            f"🔑 TAVILY_TOKEN: <b>{CFG.get('reminder_tavily_token', '30')} дн</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=admin_reminders_keyboard(),
        )

    elif data.startswith("admin_reminder_"):
        """Настройка конкретного напоминания."""
        if user_id != ADMIN_ID:
            await answer("⛔ Нет доступа", show_alert=True); return
        token_name = data.replace("admin_reminder_", "")
        valid_tokens = ["groq_token", "groq_token_2", "groq_token_3", "tavily_token"]
        if token_name not in valid_tokens:
            await answer("❌ Неверный токен", show_alert=True); return
        context.user_data["awaiting_reminder"] = token_name
        current_days = CFG.get(f"reminder_{token_name}", "30")
        await query.edit_message_text(
            f"⏰ <b>Настройка напоминания</b>\n\n"
            f"Токен: <code>{token_name.upper()}</code>\n"
            f"Текущее значение: <b>{current_days} дней</b>\n\n"
            f"Введи новое значение (число дней):",
            parse_mode=ParseMode.HTML,
        )
    ...
    elif data in ("admin_logs_requests", "admin_logs_errors", "admin_logs_payments", "admin_logs_auth"):
        if user_id != ADMIN_ID:
            await answer("⛔ Нет доступа", show_alert=True); return
        type_map = {
            "admin_logs_requests": ("📨 Логи запросов", "INFO"),
            "admin_logs_errors":   ("⚠️ Логи ошибок",   "ERROR|WARNING"),
            "admin_logs_payments": ("💳 Логи платежей",  "invoice|payment|pay"),
            "admin_logs_auth":     ("🔐 Логи авторизации", "user_id|banned|auth"),
        }
        title, keyword = type_map[data]
        try:
            import subprocess
            result = subprocess.run(
                ["grep", "-i", keyword, "cicada.log"],
                capture_output=True, text=True
            )
            lines = result.stdout.strip().split("\n")
            last = "\n".join(lines[-30:]) if lines else "Логов нет"
            text = f"{title}\n\n<pre>{last[-3500:]}</pre>"
        except Exception as e:
        ...
        await query.edit_message_text(
            ...
            f"5️⃣ Дней: <b>${p5}</b>\n"
            f"7️⃣ Дней: <b>${p7}</b>\n"
            f"1️⃣5️⃣ Дней: <b>${p15}</b>\n"
            f"3️⃣0️⃣ Дней: <b>${p30}</b>\n\n"
        ...
        # Поддерживаем периоды в днях: 1/3/5/7/15/30
        valid_plans = ("day", "week", "month", "1day", "3day", "5day", "7day", "15day", "30day")
        if plan not in valid_plans:
            await answer("❌ Неверный тариф", show_alert=True); return
        context.user_data["awaiting_price"] = plan
        # Формируем название периода
        if plan.endswith("day"):
            days = plan.replace("day", "")
            period_name = f"{days} дн"
        elif plan.endswith("month"):
            months = plan.replace("month", "")
            period_name = f"{months} мес"
        else:
            period_name = PLAN_LABELS.get(plan, plan)
        ...
        await query.edit_message_text(
            f"🤖 <b>Выбор модели</b>\n\n"
            f"⚡ llama-3.1-8b-instant — быстрый, 6K TPM\n"
            f"🧠 llama-3.3-70b-versatile — умный, 12K TPM\n"
            f"💻 qwen/qwen3-32b — для кода\n"
            f"🔭 llama-4-scout — свежий\n"
            f"🔬 deepseek-r1 — reasoning\n"
            f"🔢 qwen-qwq-32b — математика\n"
            f"🌀 gemma2-9b-it — лёгкая\n\n"
            f"<b>Текущая:</b> <code>{CFG['groq_model']}</code>",
            parse_mode=ParseMode.HTML,
...
# ════════════════════════════════════════════════════
#  /txt — получить чистый ответ от AI в виде txt файла
# ════════════════════════════════════════════════════
async def cmd_txt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Получает чистый ответ от AI и отправляет его как txt файл."""
    user_id = update.effective_user.id

    if not context.args:
        await update.message.reply_text(
            "📝 <b>Использование:</b> <code>/txt твой запрос</code>\n\n"
            "Отправляет чистый ответ от AI в виде текстового файла.",
            parse_mode=ParseMode.HTML,
        )
        return
    ...
    # Показываем "думаю..."
    wait_msg = await update.message.reply_text("💭 Получаю чистый ответ от AI...")

    try:
        # Готовим сообщения для API (без системного промпта для чистого ответа)
        api_messages = [
            {"role": "system", "content": "Ты — полезный AI ассистент. Отвечай четко и по делу."},
            {"role": "user", "content": user_input}
        ]

        # Запрос к Groq
        loop = asyncio.get_running_loop()
        _client_groq = _get_groq_client()

        def _call_groq():
            return _client_groq.chat.completions.create(
                model=CFG["groq_model"],
                messages=api_messages,
                max_tokens=4096,
            )
        ...
        filename = f"{FILES_DIR}/ai_response_{user_id}_{timestamp}.txt"
        with open(filename, "w", encoding="utf-8") as f:
        ...
        # Отправляем файл
        with open(filename, "rb") as doc_f:
            await update.message.reply_document(
                doc_f,
                filename=f"AI_Response_{timestamp}.txt",
                caption=f"📝 Чистый ответ от AI",
                parse_mode=ParseMode.HTML,
            )
...
def main():
    ...
    # Явный пул потоков: 1 поток на ~2 одновременных пользователей (Groq вызовы блокирующие)
    # При 10 пользователях одновременно — 10 потоков достаточно
    _executor = concurrent.futures.ThreadPoolExecutor(max_workers=20, thread_name_prefix="groq")

    # HTTPXRequest с отключенной SSL проверкой (фикс ошибки сертификата)
    # Патчим httpx клиент напрямую ДО создания Application
    import httpx
    ...
    # Создаем HTTPXRequest (параметры зависят от версии python-telegram-bot)
    try:
        request = HTTPXRequest(http_version="1.1")
    except TypeError:
        request = HTTPXRequest()
    # Строим Application с кастомным request (поддержка разных версий API)
    builder = Application.builder().token(BOT_TOKEN).concurrent_updates(True)
    try:
        # Для новых версий python-telegram-bot (20.0+)
        builder = builder.request_instance(request)
    except AttributeError:
        # Для старых версий - используем get_updates_request
        try:
            builder = builder.get_updates_request(request)
        except AttributeError:
            # Если ничего не работает - патча httpx достаточно
            pass
    app = builder.build()
    ...
    async def post_init(application):
        # Устанавливаем executor после запуска event loop (фикс DeprecationWarning)
        loop = asyncio.get_running_loop()
        loop.set_default_executor(_executor)
        asyncio.create_task(_auto_save_loop())
        logger.info("✅ Фоновое автосохранение БД запущено")
        logger.info(f"✅ ThreadPoolExecutor: {_executor._max_workers} потоков")
