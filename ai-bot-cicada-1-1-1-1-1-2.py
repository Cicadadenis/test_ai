...
def trim_messages_to_token_limit(messages: list, max_chars: int = 18000) -> list:
    """
    Обрезает список сообщений чтобы суммарный объём не превышал max_chars.
    Системное сообщение (role=system) всегда сохраняется.
    Удаляет старые сообщения из середины (не трогая последнее user-сообщение).
    """
    total = sum(len(m.get("content", "")) for m in messages)
    if total <= max_chars:
        return messages

    # Разделяем: system + история + последнее сообщение
    ...
    result = system_msgs + mid_msgs + [last_msg]
    trimmed = len(messages) - len(result)
    if trimmed > 0:
        logger.info(f"trim_messages: удалено {trimmed} сообщений (было {total} символов)")
    return result
...
def extract_files_from_reply(reply: str) -> dict:
    ...
    Ищет имя файла перед блоком кода или в первой строке-комментарии.
    """
    files = {}
    # Паттерн: имя файла на строке прямо перед блоком кода
    lines = reply.split('\n')
    i = 0
    code_blocks = []
    while i < len(lines):
        ...
        if lines[i].startswith('```'):
            ...
            code_blocks.append({'lang': lang, 'code': code, 'prev_line': lines[i-len(code_lines)-1] if i-len(code_lines)-1 >= 0 else ''})
        i += 1
    ...
    for idx, block in enumerate(code_blocks):
        ...
        code = block['code']
        prev = block['prev_line'].strip()

        # Ищем имя файла в предыдущей строке
        fname_match = re.search(r'([\w\-]+\.\w{1,6})', prev)
        if fname_match:
            fname = fname_match.group(1)
        else:
            # Ищем в первой строке кода (комментарий)
            first = code.split('\n')[0] if code else ''
            fname_match2 = re.search(r'([\w\-]+\.\w{1,6})', first)
            if fname_match2:
                fname = fname_match2.group(1)
            else:
