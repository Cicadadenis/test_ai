            ...
            if reply is None:
                ...
                for _attempt in range(len(_GROQ_KEYS) or 1):
                    ...
                    try:
                        ...
                        last_exc = _e
                        err_str = str(_e)
                        is_token_limit = "413" in err_str or ("tokens" in err_str and "limit" in err_str.lower())
                        is_rate_limit  = "429" in err_str or "rate_limit" in err_str.lower()
                        if is_token_limit:
                            # 413 — запрос слишком большой, сразу пробуем следующий ключ
                            logger.warning(f"[{user_id}] Groq ключ {_attempt+1} — 413 token limit, переключаем ключ")
                        elif is_rate_limit:
                            logger.warning(f"[{user_id}] Groq ключ {_attempt+1} ошибка: {_e}, ждём 15с")
                            await asyncio.sleep(15)
                        else:
                            logger.warning(f"[{user_id}] Groq ключ {_attempt+1} ошибка: {_e}, ждём 2с")
